    ...
    if cov:
        ...
        if cov.get("kg_built") is False:
            st.info(
                ...
                "Tracked in #61 — search will hit the positional fallback."
            )
        else:
...
def render_kg_coverage(run: dict[str, Any]) -> None:
    ...
    if cov.get("kg_built") is False:
        st.info(
            ...
            f"`({cov.get('merchant_id')}, {cov.get('kg_strategy')})` — "
            "tracked in #61.\n\n"
            "Freshly-POSTed merchants inherit default's enriched rows but "
            "start without `:Product` nodes. Search will hit the positional "
            "fallback at `merchant_agent.py` until the KG is built."
        )
        return
    cols = st.columns(4)
