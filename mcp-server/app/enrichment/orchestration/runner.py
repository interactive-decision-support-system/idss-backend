...
class RunSummary:
    ...
    # nodes yet — the "fresh merchant, tracked in #61" path.
...
def _compute_kg_reader_coverage(
    *,
    ...
    the fresh-merchant path from POST /merchant (tracked in #61) — return
    a sentinel with ``kg_built=False`` and skip the drift table so the
    inspector can surface an "N/A" banner instead of a false-positive red
    list.
    """
...
def _kg_has_products(*, merchant_id: str, kg_strategy: str) -> bool:
    ...
    shows the "KG not built yet — #61" banner rather than exploding."""
    try:
