...
# Deterministic UUID5 test product IDs
_NS = uuid.NAMESPACE_DNS
LAPTOP_001 = uuid.uuid5(_NS, "test-mcp-pipeline-laptop-001")
LAPTOP_002 = uuid.uuid5(_NS, "test-mcp-pipeline-laptop-002")
...
def setup_database():
    """Create fresh database records for each test."""
    # Re-apply our override so we use the correct PostgreSQL connection
    app.dependency_overrides[get_db] = override_get_db
    Base.metadata.create_all(bind=engine)

    # Clear in-memory carts that may persist between test modules
    _CARTS.clear()

    db = TestingSessionLocal()
    test_uuids = [LAPTOP_001, LAPTOP_002, BOOK_001]
    ...
    # Create test products using new Supabase schema
    products = [
        Product(
        ...
        Product(
            ...
            price_value=999.99,
            inventory=10,
            attributes={"description": "Business laptop for productivity"}
        ),
        Product(
            product_id=BOOK_001,
            name="Mystery Novel",
            ...
            price_value=19.99,
            inventory=20,
            attributes={"description": "Thrilling mystery novel"}
        )
    ]
    ...
    # Cleanup: delete test products by UUID
    db = TestingSessionLocal()
    for uid in test_uuids:
        db.query(Product).filter(Product.product_id == uid).delete(synchronize_session=False)
    db.commit()
...
class TestGetProduct:
    ...
    def test_get_product_mcp_protocol(self):
        ...
        response = client.post(
            "/api/get-product",
            json={"product_id": str(LAPTOP_001)}
        )
        ...
        assert data["data"]["product_id"] == str(LAPTOP_001)
    ...
    def test_get_product_ucp_protocol(self):
        ...
        assert data["product"]["id"] == str(LAPTOP_001)
        assert latency < 1000
...
class TestAddToCart:
    ...
    def test_add_product_to_cart(self):
        ...
        assert data["data"]["items"][0]["product_id"] == str(LAPTOP_001)
    ...
    def test_add_to_cart_out_of_stock(self):
        ...
        oos_uuid = uuid.uuid5(_NS, "test-mcp-pipeline-out-of-stock-001")

        # Create a product with 0 inventory
        db = TestingSessionLocal()
        try:
            # Clean up first in case it exists
            db.query(Product).filter(Product.product_id == oos_uuid).delete(synchronize_session=False)
            db.commit()
            ...
            product = Product(
                ...
                price_value=500.00,
                inventory=0,
                attributes={"description": "Out of stock test product"}
            )
            db.add(product)
            db.commit()
        finally:
        ...
        response = client.post(
            ...
            json={
                ...
                "product_id": str(oos_uuid),
                "qty": 1
            }
        )

        # Cleanup the OOS product
        db = TestingSessionLocal()
        try:
            db.query(Product).filter(Product.product_id == oos_uuid).delete(synchronize_session=False)
            db.commit()
...
class TestProtocolCompatibility:
    ...
    def test_mcp_to_ucp_response_format(self):
        ...
        ucp_response = client.post(
            ...
            json={
                "action": "get_product",
                "parameters": {"product_id": str(LAPTOP_001)}
            }
        )
...
class TestResponseEnvelope:
    ...
    def test_response_always_has_trace(self):
        ...
        endpoints = [
            ("/api/get-product", {"product_id": str(LAPTOP_001)}),
            ("/api/search-products", {"query": "laptop", "limit": 10}),
            ("/api/add-to-cart", {"cart_id": "test", "product_id": str(LAPTOP_001), "qty": 1})
        ]
