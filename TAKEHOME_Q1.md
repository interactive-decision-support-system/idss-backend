...
|---|-------------|------|--------|---------------|
| 1 | **Compare** | `agent/chat_endpoint.py` | Added `"lay these out"` to `_FAST_COMPARE_KWS` | Deterministic substring, zero latency, zero LLM cost. `"these"` and `"them"` are distinct tokens; both must be listed. |
