...
On aggregate score, MACS is statistically tied with catalog-aware LLM baselines,
but achieves zero constraint drift and stronger reliability on the hardest
state-management scenarios. These results show that reliable conversational
...
filter compliance. On a 10-scenario multi-turn benchmark, MACS and \GPTCatalog{}
\GPTCatalog{} leads on aggregate multi-turn score (0.747 vs.\ 0.721) and
macro Pass@5 (54\% vs.\ 50\%); however, MACS uniquely achieves 100\% Pass@5
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrrr@{}}
    ...
    \midrule
    MACS             & \textbf{0.681} & \textbf{87.1\%} & 0.721$\pm$0.115 & 50\%          & \textbf{0.000} \\
    \GPTCatalog{}    & 0.639          & 72.1\%          & \textbf{0.747}$\pm$0.088 & \textbf{54\%} & \textbf{0.000} \\
...
(K=5 runs, threshold $\geq 0.65$). \GPTCatalog{} leads on aggregate multi-turn score (0.747 vs.\ 0.721 for MACS
vs.\ 0.727 for Gemini) and macro Pass@5 (54\% vs.\ 50\% vs.\ 52\%).
However, MACS uniquely achieves 100\% Pass@5 on Exclusion Reversal---the
...
pathway. Budget overwrite achieves 100\% for both MACS and \GeminiCatalog{}
(vs.\ 80\% for \GPTCatalog{}).
\GPTCatalog{} outperforms on long-session and clarification scenarios
(100\% each), where a well-structured single-turn LLM call suffices without
accumulated constraint state. All three systems achieve zero constraint drift
across all 50 run-scenario pairs in this run.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    \midrule
    Brand Exclusion       & \checkmark 0.703 / 80\%  & 0.757 / 40\%             & 0.640 / 0\%   \\
    Budget Overwrite      & \checkmark 0.793 / 100\% & \checkmark 0.757 / 80\%  & \checkmark 0.838 / 100\% \\
    Budget Refinement     & \checkmark 0.802 / 80\%  & \checkmark 0.802 / 80\%  & \checkmark 0.838 / 100\% \\
    Clarification         & \checkmark 0.757 / 80\%  & \checkmark 0.856 / 100\% & \checkmark 0.802 / 100\% \\
    Comparison            & 0.613 / 0\%              & 0.640 / 0\%              & 0.604 / 0\%   \\
    Constraint Persist.   & 0.640 / 0\%              & 0.748 / 40\%             & 0.685 / 40\%  \\
    Dense Constraint      & \checkmark 0.721 / 60\%  & \checkmark 0.802 / 80\%  & \checkmark 0.874 / 80\%  \\
    Exclusion Reversal    & \checkmark 0.964 / 100\% & 0.622 / 20\%             & 0.586 / 0\%   \\
    Intent Pivot          & 0.640 / 0\%              & 0.631 / 0\%              & 0.613 / 0\%   \\
    Long Session          & 0.577 / 0\%              & \checkmark 0.856 / 100\% & \checkmark 0.793 / 100\% \\
    \midrule
    Overall               & 0.721 / 50\%             & \textbf{0.747 / 54\%}    & 0.727 / 52\%  \\
    \bottomrule
  \end{tabular}
\end{table}
...
Budget overwrite is strong for both MACS and Gemini (100\%) but not GPT (80\%).
\GPTCatalog{} leads on long session and clarification, where accumulated
state is less critical. Dense constraint and comparison remain hard for all
systems; intent pivot and comparison score 0\% across all three, confirming
...
recommendation in fixed-catalog settings. The central finding is not that
the system produces better aggregate language output---on mean multi-turn
score, \GPTCatalog{} (0.747) leads MACS (0.721) and both are within one SD.
The finding is that separating interaction from enforcement yields
measurable reliability advantages on the hardest scenarios: 100\% Pass@5 on
exclusion reversal where prompt-based baselines score 20\% and 0\%,
100\% Pass@5 on brand exclusion persistence where they score 40\% and 0\%,
and a disclosure score 0.338 points above the nearest baseline on
catalog-impossible queries. These advantages
were not observed in any prompt-based baseline evaluated in this work; they
follow from session-persisted constraint state and SQL enforcement rather than
from more careful prompting.
...
multi-turn exclusion-reversal Pass@5=100\% (GPT 20\%, Gemini 0\%), zero constraint drift
